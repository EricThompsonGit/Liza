...
public class Navigation {
            ...
            final double angularStepRad)
    {
        ...
        final double distance = shipPosition.getDistanceTo(targetPos);
        final double angleRad = shipPosition.orientTowardsInRad(targetPos);

        if (avoidObstacles && !gameMap.planetsBetween(shipPosition, targetPos).isEmpty()) {
            final double newTargetDx = Math.cos(angleRad + angularStepRad) * distance;
            final double newTargetDy = Math.sin(angleRad + angularStepRad) * distance;
            final Position newTarget = new Position(shipPosition.getXPos() + newTargetDx, shipPosition.getYPos() + newTargetDy);

            return navigateShipTowardsTarget(gameMap, ship, shipPosition, newTarget, maxThrust, true, (maxCorrections-1), angularStepRad);
        }
