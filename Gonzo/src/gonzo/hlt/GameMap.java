...
//    	Log.log("  PlayerIds = " + players.keySet());
    ...
    public ArrayList<Entity> objectsBetween(Position start, Position target) {
        ...
        addEntitiesBetween(entitiesFound, start, target, planets.values(), Constants.FORECAST_FUDGE_FACTOR);
        addEntitiesBetween(entitiesFound, start, target, allShips.values(), Constants.FORECAST_FUDGE_FACTOR);
    ...
    public ArrayList<Entity> planetsBetween(Position start, Position target) {
        ...
        addEntitiesBetween(entitiesFound, start, target, planets.values(), Constants.FORECAST_FUDGE_FACTOR+1);
    ...
    private static void addEntitiesBetween(final List<Entity> entitiesFound,
                                           ...
                                           double dFudgeFactor ) { // Constants.FORECAST_FUDGE_FACTOR

        for (final Entity entity : entitiesToCheck) {
            if (entity.equals(start) || entity.equals(target)) {
                continue;
            }
            if (Collision.segmentCircleIntersect(start, target, entity, dFudgeFactor)) {
